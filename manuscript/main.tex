...
\usepackage{hyperref}
\usepackage{xcolor}
% \usepackage{siunitx}  % removed: version conflicts with MiKTeX 22.1
...
The global installed PV capacity exceeded 1.6\,TW in 2024,
...
efficiency by 10\%--40\%, and create fire hazards
...
\Cref{fig:pipeline} provides an overview of the complete experimental
pipeline from data acquisition to cross-domain validation.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/methodology_flowchart.png}
  \caption{End-to-end methodology pipeline: IRT data collection
           $\rightarrow$ pre-processing $\rightarrow$ model selection
           (SDCNN / VGG-16 / MobileNetV2 / EfficientNet-B0)
           $\rightarrow$ 5-fold CV $\rightarrow$ statistical evaluation
           $\rightarrow$ cross-domain validation.}
  \label{fig:pipeline}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/sdcnn_tikz.pdf}
...
\Cref{fig:cross_domain_pipeline} illustrates the evaluation pipeline.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/cross_domain_pipeline.png}
  \caption{Zero-shot cross-domain evaluation pipeline. Weights trained solely
           on the Ghana dataset are applied directly to the Pierdicca Italian
           PV dataset with no fine-tuning. Domain differences (climate,
           camera, altitude) induce a measurable F1 gap.}
  \label{fig:cross_domain_pipeline}
\end{figure}
